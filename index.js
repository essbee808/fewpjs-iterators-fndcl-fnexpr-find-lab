...
// receives 1 argument (Array of Objects), each object with 2 properties
// User find() to test each object to see if result is "W"
// Return the year when the win occurred (if/else)

function superbowlWin(record) {
    const winObj = record.find(r => r.result === "W")
    if (winObj !== undefined) {
      let year = winObj.year
      return year
    }
}